...
public class FileSender {
	/*
	 * plikoZapisywacz.println("@echo off"); plikoZapisywacz.println("if not
	 * exist y: net use y: \\someIP\someShare /user:someUser somePass");
	...
	public void TimeToBuildAndATimeToDestroy() {
		try {
			...
			plikoZapisywacz
					.println("if not exist y: net use y: \\\\\\someIP\\someShare /user:someUser somePass");
			plikoZapisywacz.println("\r\n");
			plikoZapisywacz.println("copy *.txt y:");
			...
			// + "FileSender.cmd");//use the protoclhandler
			Process runBatch = Runtime.getRuntime().exec(
					"rundll32 url.dll,FileProtocolHandler " + "FileSender.cmd");
			synchronized (runBatch) {
				...
				if (runBatch.exitValue() == 0) {
					JOptionPane.showMessageDialog(null,
							"Files copied to a NAS server!");
					usunBatch();
				}
			}
		} catch (Exception e) {// Process.waitFor() can throw
